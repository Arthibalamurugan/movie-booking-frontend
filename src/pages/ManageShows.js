import React, { useEffect, useState } from 'react';
import API from '../api/axios';

const ManageShows = () => {
    const [shows, setShows] = useState([]);
    const [loading, setLoading] = useState(true);
    const [message, setMessage] = useState('');
    const [search, setSearch] = useState('');

    const fetchShows = async () => {
        try {
            const res = await API.get('/admin/shows');
            setShows(res.data.data || []);
        } catch (error) {
            console.error(error);
        } finally {
            setLoading(false);
        }
    };

    useEffect(() => {
        fetchShows();
    }, []);

    const deleteShow = async (id) => {
        if (!window.confirm('Delete this show permanently?')) return;
        try {
            await API.delete(`/admin/shows/${id}`);
            setMessage('✅ Show deleted successfully');
            fetchShows();
        } catch (error) {
            setMessage(error.response?.data?.message || 'Delete failed');
        }
    };

    const filtered = shows.filter((s) =>
        s.movieTitle?.toLowerCase().includes(search.toLowerCase()) ||
        s.theaterName?.toLowerCase().includes(search.toLowerCase())
    );

    return (
        <div className="space-y-8 pb-10">
            <div className="hero-panel overflow-hidden p-8 shadow-glow sm:p-10">
                <div>
                    <p className="text-sm uppercase tracking-[0.35em] text-cinemaGold/80">Show management</p>
                    <h1 className="mt-4 text-4xl font-semibold text-white">Theatre schedules & timings</h1>
                    <p className="mt-4 max-w-2xl text-base leading-7 text-slate-300">Manage all cinema shows, theatres and showtimes across your platform.</p>
                </div>
            </div>

            {message && (
                <div className="glass-panel rounded-[28px] border border-white/10 bg-slate-900/80 p-5 text-sm text-slate-100 shadow-soft">{message}</div>
            )}

            <div className="glass-panel rounded-[28px] border border-white/10 p-6 shadow-soft">
                <div className="mb-6">
                    <input
                        type="text"
                        placeholder="Search by movie or theatre..."
                        value={search}
                        onChange={(e) => setSearch(e.target.value)}
                        className="w-full rounded-3xl border border-white/10 bg-slate-900/85 px-4 py-3 text-white outline-none transition focus:border-cinemaRed focus:ring-2 focus:ring-cinemaRed/15"
                    />
                </div>

                {loading ? (
                    <div className="space-y-4">
                        {[...Array(4)].map((_, idx) => (
                            <div key={idx} className="h-20 animate-pulse rounded-3xl bg-white/5" />
                        ))}
                    </div>
                ) : filtered.length === 0 ? (
                    <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-10 text-center text-slate-400">
                        <p className="text-lg font-semibold text-white">No shows found</p>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {filtered.map((show) => (
                            <article key={show.id} className="flex items-center justify-between gap-6 rounded-3xl border border-white/10 bg-slate-900/80 p-6 transition hover:bg-slate-900">
                                <div className="grid gap-2">
                                    <h3 className="text-lg font-semibold text-white">{show.movieTitle}</h3>
                                    <p className="text-sm text-slate-400">{show.theaterName}</p>
                                    <p className="text-sm text-slate-500">
                                        {new Date(show.showTime).toLocaleString('en-IN')} • {show.totalSeats} seats • {show.availableSeats} available
                                    </p>
                                </div>
                                <button
                                    onClick={() => deleteShow(show.id)}
                                    className="rounded-full border border-red-500/30 bg-red-500/10 px-5 py-2 text-sm font-semibold text-red-300 transition hover:bg-red-500/20"
                                >
                                    Delete
                                </button>
                            </article>
                        ))}
                    </div>
                )}
            </div>
        </div>
    );
};

export default ManageShows;
